...
def train_model(params, epoches=1000, warmup_epochs=50):
    ...
    for i in range(min(len(params['src_chars']),len(params['tgt_chars']))): #캐릭터 조합에 따른 for 분기
        dataloader, joint_mapping = create_dataloader(params['src_chars'], params['tgt_chars'])
        ...
        for epoch in tqdm(range(epoches), desc="Epoch", leave=False): #epoch 설정에 따른 for 분기
            _, tgt_vert_pos, tgt_geo, geo_e, tgt_skel = dataloader.__getitem__(0)

            if FirstRun:
                    model = BaseModel(input_size, tgt_vert_pos, tgt_skel, tgt_geo, 64, geo_e) 
                    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
                    FirstRun = False
            else:
                model.changeconfig(tgt_vert_pos, tgt_skel, tgt_geo, geo_e)

            batch_sequences, _, _, _, _ = dataloader.__getitem__(k) #각 애니메이션 클립으로부터 만들어진 batch로 인한 분기
            k = (k+1) % num_motion
